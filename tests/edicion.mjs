...
console.log('Edición: ambientes, navegación, menús plegables y confirmación de competición: OK');
{
  ...
  try {
    ...
    assert.equal(w.document.querySelector('.screen-enter'), null, 'el menú del mazo no entra desplazándose lateralmente');
    click(w, '[data-action="solo"]');
    assert.equal(w.document.getElementById('app').dataset.screen, 'solo-home');
    assert.equal(w.document.querySelector('.camera-move, .screen-enter'), null, 'solitario muestra directamente su única pantalla real');
    const panels = [...w.document.querySelectorAll('.solo-panel')].map(panel => panel.textContent.trim());
    assert.ok(panels[0].includes('Reto diario') && panels[1].includes('Partida libre'), 'las opciones de solitario llegan juntas y estables');
  } finally { w.close(); }
}
console.log('Preparación estable: una sola pantalla, sin desplazamientos ni vistas provisionales: OK');
...
console.log('Atajo al inicio: marca dibujada, caja propia y especificidad que gana a la barra agrupada: OK');
{
  ...
  try {
    ...
    assert.equal(turns.length, 0, 'el mazo y la preparación no crean capas de cámara');
    for (const screen of ['game', 'pass', 'game']) render(screen);
    assert.equal(turns.length, 0, 'las jugadas tampoco crean capas globales');
    render('home'); render('play-menu'); render('solo-home'); render('solo');
    assert.equal(turns.length, 0, 'solitario entra directamente');
    render('home'); render('comp-intro'); render('solo');
    assert.equal(turns.length, 0, 'competición entra directamente');
    render('home'); render('play-menu'); render('online-loading'); render('online-entry'); render('online-lobby'); render('online-game');
    assert.equal(turns.length, 0, 'la preparación online tampoco duplica pantallas');
    render('home'); render('play-menu'); render('setup'); render('play-menu');
    assert.equal(turns.length, 0);
    assert.equal(w.document.querySelector('.camera-move, .screen-enter'), null);
  } finally { w.close(); }
}
console.log('Preparación completa sin capas transitorias ni dobles entradas: OK');
{
  const w = boot({ seen: true });
  const effects = [];
  try {
