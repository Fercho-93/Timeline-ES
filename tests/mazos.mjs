// Reglas de calidad de los tres mazos: sin cartas repetidas, sin huecos y, sobre todo,
...
globalThis.window = {};
for (const archivo of ["cards.js", "movies.js", "countries.js"]) {
  new Function(fs.readFileSync(path.join(REPO, archivo), "utf8")).call(globalThis);
}
const { HISTORY_CARDS, MOVIE_CARDS, COUNTRY_CARDS } = globalThis.window;
let fail = 0;
...
comunes("Superficie de países", COUNTRY_CARDS, c => c.value);
const orden = [...COUNTRY_CARDS].sort((a, b) => b.value - a.value);
...
ok("superficies sin repetir", new Set(COUNTRY_CARDS.map(c => c.value)).size === COUNTRY_CARDS.length);
ok("todas las superficies son positivas", COUNTRY_CARDS.every(c => c.value > 0));

const todos = [...HISTORY_CARDS, ...MOVIE_CARDS, ...COUNTRY_CARDS];
console.log("\nEntre mazos");
