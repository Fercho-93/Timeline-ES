(function () {
  ...
  function restoreView() {
    ...
    routes.cifras = CifrasMode.resumeCifras;
  ...
  function duelPanel() {
    ...
    const enCifras = CifrasMode.cargaCifras();
  ...
  // que puntúa y lo que viaja en el enlace está en duelo.js. Lo propio de cifras —su
  // estado y sus pantallas— vive en app-cifras.js (C.2.1: es el subsistema más
  // autocontenido del motor local); lo que sigue siendo del duelo en general —sobre
  // todo el reloj, la regla de la que depende todo lo demás— se queda aquí.
  //
  ...
  function leeCifra(texto, modeKey = selectedModeKey) { return Cifras.leer(modeKey, texto); }

  const CifrasMode = CT.createCifrasMode({
    ...
    setLastDuelShare: value => { lastDuelShare = value; }
  });
  ...
  function cartaEnReloj() {
    if (screen === "cifras") return CifrasMode.abierta();
  ...
  function duelPlay() {
    ...
    duelPreparado = null;
    cuentaAtras(() => (modalidad === "cifras" ? CifrasMode.startCifras(duel) : startSolo("duel", duel)));
  }
  ...
  app.addEventListener("click", event => {
    ...
    else if (action === "resume-cifras") CifrasMode.resumeCifras();
    else if (action === "cifra-answer") CifrasMode.cierraCarta("respuesta");
    else if (action === "cifras-next") CifrasMode.cifrasNext();
    else if (action === "cifras-exit") CT.UI.confirmExit("La carta que tengas abierta se cerrará: el reloj no se para.", () => CifrasMode.salir());
