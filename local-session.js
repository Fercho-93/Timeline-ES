...
(function () {
  ...
  function actionFromMessage(message, context) {
    const data = message?.data || {}; const { modeKey, valueOf, shuffle, now } = context;
    switch (message?.type) {
  ...
  function createHostSession({ roomCode, hostName, avatarId = null, modeKey, deckFingerprint = null, now = () => Date.now(), onChange }) {
    const valueOf = valueOfForMode(modeKey); const context = { modeKey, valueOf, shuffle: CT.shuffle, now };
    let room = CT.LocalRoom.createRoom({ roomCode, hostId: HOST_ID, hostName, avatarId, modeKey, deckFingerprint, now: now() }); onChange(room);
    ...
    let lan = null, lanSecret = null; const pendingPeers = new Map();
    ...
    async function ensureLan() {
      if (!CT.LocalLanSignal?.available?.()) return null; if (lan) return lan; lanSecret = CT.LocalLanSignal.token();
      lan = await CT.LocalLanSignal.startHost({ token: lanSecret, onAnswer: async (answer, peerId) => {
        const pending = pendingPeers.get(String(peerId || "")); if (!pending) return;
        try { await pending.acceptAnswer(answer); pendingPeers.delete(String(peerId)); } catch (error) { console.error("LAN answer rejected", error); }
      }}); return lan;
    }
    async function invitePeer() {
      const peer = transport.addPeer(); const rawOffer = await peer.offerSignal();
      try {
    ...
    function close() { try { transport.closeAll(); } finally { pendingPeers.clear(); if (lan) void lan.stop(); lan = null; lanSecret = null; } }
  ...
  function createGuestSession({ offerSignal, playerId, name, avatarId = null, deckFingerprint = null, onChange, onError }) {
    ...
    const peer = CT.LocalTransport.createGuestPeer(rawOffer, message => {
      if (message.type === "state") { room = message.data; onChange(room); } else if (message.type === "error") onError?.(message.data?.message);
    ...
    return {
      ...
      placeCard: (cardId, index) => peer.send("place-card", { playerId, cardId, index }), finishTurn: () => peer.send("finish-turn", { playerId }), skipTurn: () => peer.send("skip-turn", { playerId }),
      removePlayer: targetId => peer.send("remove-player", { playerId, targetId }), currentRoom: () => room, close: peer.close, automaticLan: !!lanInvite
    };
  }
