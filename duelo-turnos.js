...
function render() {
  ...
  const html = `<div class="shell turn-duel-shell" data-duel-id="${safe(current.id)}" data-turn="${current.turnIndex}">
    <nav class="turn-duel-nav" aria-label="Duelo"><span>CONTINUUM <small>Duelo por turnos</small></span><div class="turn-duel-nav-actions"><i data-sound-slot></i><button class="btn btn-ghost" data-turn-action="back">Volver</button></div></nav>
    <section class="turn-duel-screen">
      ...
      <div class="turn-duel-scores" aria-label="Marcador">${current.playersOrder.map((player, index) => { const name = current.players?.[player]?.alias || 'Jugador'; const mine = player === uid(); const activePlayer = current.turnUid === player; return `<div class="turn-duel-player ${activePlayer ? 'is-active' : ''} ${mine ? 'is-you' : ''}" data-player="${safe(player)}"><span class="turn-duel-avatar" aria-hidden="true">${CT.Avatares?.markup(name, { size: 44, seed: 'uid:' + player }) || safe(name.trim().charAt(0).toUpperCase() || '?')}</span><span class="turn-duel-player-info"><strong>${safe(name)}</strong><small>${mine ? 'Tú' : index === 1 ? 'Rival' : 'Jugador'}</small></span><span class="turn-duel-score"><b>${current.scores?.[player] || 0}</b><small>${current.kind === 'cifras' ? 'puntos' : 'aciertos'}</small></span>${activePlayer ? '<span class="turn-duel-turn-badge">Turno</span>' : ''}</div>`; }).join('')}</div>
