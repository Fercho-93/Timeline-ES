// Protocolo de señalización LAN para el prototipo de multijugador offline con un solo QR.
// La capa nativa (iOS/Android) debe exponer window.Capacitor.Plugins.LocalLanSignal.
(function () {
  "use strict";
  ...
  function encodeInvite({ host, port = DEFAULT_PORT, token: secret, roomCode, modeKey, deckFingerprint, signal }) {
    if (!host || !secret || !roomCode || !signal) throw new Error("INVALID_LAN_INVITE");
    return "CTL1:" + JSON.stringify({ v: VERSION, host, port, token: secret, roomCode, modeKey, deckFingerprint, signal });
  }
  function decodeInvite(text) {
    ...
    if (data?.v !== VERSION || !data.host || !data.port || !data.token || !data.roomCode || !data.signal) throw new Error("INVALID_LAN_INVITE");
    return data;
  }
  ...
  async function sendAnswer(invite, answer, peerId) {
    const p = plugin(); if (!p) throw new Error("LAN_SIGNAL_UNAVAILABLE");
    return p.sendAnswer({ host: invite.host, port: invite.port, token: invite.token, answer, peerId: peerId || "" });
  }
