...
**No está desplegado ni conectado a las pantallas de la aplicación.** No ofrece todavía Fantasma, abandono/expulsión, recuperación de anfitrión ni resolución por tiempo: una desconexión conserva la partida, pero no sustituye al participante. No debe anunciarse como una modalidad pública disponible.
...
Acciones: `create` con `catalog`; `join`; `start`; `play` con `cardId,index`; `pulse` con `target`; `answer` con `index`; `endTurn`.
