...
Acciones: `create` con `catalog`; `join`; `start`; `play` con `cardId,index`; `pulse` con `target`; `answer` con `index`; `endTurn`; `leave`; `heartbeat` para registrar presencia; `claimHost` tras 90 segundos sin señal del anfitrión; `timeout` tras 45 segundos sin actividad del turno.
